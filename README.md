...
├── pnl_registry.py        # Read-only reporting — derives trades.csv and daily_summary.csv
├── requirements.txt
├── .env.template
├── .gitignore
└── data/
    ...
    ├── trades.csv             # Per-trade report (regenerated by pnl_registry.py)
    └── daily_summary.csv      # Per-day aggregated metrics (regenerated by pnl_registry.py)
```
